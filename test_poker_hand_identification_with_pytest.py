...
def test_id1():
    ...
    assert hand.player_dict[1].best_hand == 'One Pair'
    assert hand.player_dict[1].get_hand_value() == 2


def test_id2():
    hand = texas_poker.PokerTable(board=['2H', '4S', '6C', '8D', 'TS'], player1_hand=['AH', 'KD'])
    assert hand.player_dict[1].best_hand == 'High Card'
    assert hand.player_dict[1].get_hand_value() == 1


def test_id3():
    hand = texas_poker.PokerTable(board=['AH', 'AS', 'AC', 'AD', 'TS'], player1_hand=['4C', '5C'])
    assert (hand.player_dict[1].best_hand == '4 Of A Kind')
    assert hand.player_dict[1].get_hand_value() == 8


def test_id4():
    hand = texas_poker.PokerTable(board=['8H', '8S', '8C', '3D', '3S'], player1_hand=['7C', '9C'])
    assert (hand.player_dict[1].best_hand == 'Full House')
    assert hand.player_dict[1].get_hand_value() == 7


def test_id5():
    hand = texas_poker.PokerTable(board=['AH', 'AS', 'AC', '8D', 'TS'], player1_hand=['7C', '3H'])
    assert (hand.player_dict[1].best_hand == '3 Of A Kind')
    assert hand.player_dict[1].get_hand_value() == 4


def test_id6():
    hand = texas_poker.PokerTable(board=['AS', 'KS', 'QS', 'JS', 'TS'], player1_hand=['2C', '3C'])
    assert (hand.player_dict[1].best_hand == 'Royal Flush')
    assert hand.player_dict[1].get_hand_value() == 10


def test_id7():
    hand = texas_poker.PokerTable(board=['3H', '4S', '5C', '6D', '7S'], player1_hand=['AH', 'QH'])
    assert (hand.player_dict[1].best_hand == 'Straight')
    assert hand.player_dict[1].get_hand_value() == 5
...
def test_id9():
    ...
    assert (hand.player_dict[1].best_hand == 'Two Pair')
    assert hand.player_dict[1].get_hand_value() == 3
...
def test_id12():
    ...
    assert (hand.player_dict[1].best_hand == 'Flush')
    assert hand.player_dict[1].get_hand_value() == 6
...
def test_id20():
    ...
    assert (hand.player_dict[1].best_hand == 'Straight Flush')
    assert hand.player_dict[1].get_hand_value() == 9
